...
import { AdminTabType } from '@/app/constans/tab';
...
const Header = () => {
  ...
  const currentTab =
    (searhchParams.get('tab') as AdminTabType) ?? AdminTabType.All;

  const handleTabClick = (option: AdminTabType) => {
    router.push(`/admin?tab=${option}`);
  };
  ...
  return (
    <header className="bg-gray_90 fixed top-0 z-50 flex w-full items-center justify-center">
      <div className="tems-center grid h-52 w-full max-w-1440 grid-cols-3 px-24 py-10">
        <Image
          alt="header logo"
          src="/icons/logo.svg"
          width={101}
        ...
        <div className="flex justify-center">
          <Tab handleClick={handleTabClick} currentOption={currentTab} />
        </div>
        <div className="flex items-center justify-end gap-36">
